...
#(
    ...
    parameter SIM_COMPONENT_CALL_COUNT_LOG_FILE = "sim_component_call_count.log"
    )
    (
    ...
    import "DPI-C" context function int __ihc_hls_get_component_call_count(string component_name);
    ...
    int component_done_count[0:NUM_COMPONENTS-1]='{default:0};
    ...
    function void dump_component_call_count_info();
        int i;
        int DUMP_FILE;

        if (SIM_COMPONENT_CALL_COUNT_LOG_FILE != "") begin
            DUMP_FILE = $fopen(SIM_COMPONENT_CALL_COUNT_LOG_FILE);

            $sformat(message, "\n");
            __ihc_hls_dbgs(message);
            for (i=0;i<NUM_COMPONENTS;i++)
            begin
                automatic int component_call_count = __ihc_hls_get_component_call_count(component_names_arr[i]);
                $sformat(message, "[%7t][sim][main_dpi_ctrl] component_call_count[%s] = %0d",$time, component_names_arr[i], component_call_count);
                __ihc_hls_dbgs(message);
                $fdisplay(DUMP_FILE, "component_call_count\t%s\t%0d", component_names_arr[i], component_call_count);
            end
            $sformat(message, "\n");
            __ihc_hls_dbgs(message);
    ...
    begin
        ...
        dump_component_call_count_info();
    ...
    initial begin
        forever begin
            @(posedge clock) begin
                if (|component_done) begin
                    for (int i = 0; i < NUM_COMPONENTS; ++i) begin
                        component_done_count[i] = component_done_count[i] + component_done[i];
                    end
                end
            end
        end
    ...
    task automatic __ihc_hls_component_wait_sim_task(string component_name, int component_call_count);
        int i;
        int component_index;
        ...
        $sformat(message, "[%7t][msim][dut_task] component_done = %0d", $time, component_done_count[component_index]);
        __ihc_hls_dbgs(message);
        $sformat(message, "[%7t][msim][dut_task] waiting for component_done == %0d", $time, component_call_count);
        __ihc_hls_dbgs(message);

        assert (component_call_count >= component_done_count[component_index]) 
          else $fatal(1, "[sim] Component sent return data more times than it was invoked!");
        wait( component_done_count[component_index] == component_call_count );
